...
void user_algorithm(GraphCSR _graph, int *_levels, int *_frontier_ids, int *_bunched_ids, int *_low_ids, const vector<cudaStream_t>& Stream, int _source_vertex)
{
    ...
    gpu_bfs_wrapper(_graph.outgoing_ptrs, _graph.outgoing_ids, _graph.vertices_count, _graph.edges_count, _source_vertex, device_levels, _frontier_ids, _bunched_ids, _low_ids, Stream);
...
void convert_edges_list_to_CSR(Graph &_edges_list_graph, GraphCSR &_csr_graph)
{
    ...
    _csr_graph.outgoing_ptrs = new long long[vertices_count + 1];
...
int main(int argc, char **argv)
{
	try
	{
        ...
        int *user_result;
        cudaMallocHost((void**)&user_result, deviceCount*sizeof(int)*graph.vertices_count);
        double t_start = omp_get_wtime();

        // выделяем память под ответ
        // двойной длины для каждой видеокарты
        int gpu_device = 0;
        int gpu_last_source = 0;
        #pragma omp parallel num_threads(deviceCount)
        {
            int device = omp_get_thread_num();
            printf("device: %d\n", device);
            cudaSetDevice(device);
            long offset = device*graph.vertices_count;
            GraphCSR gpu_graph;
            ...
            int *frontiers_id;
            SAFE_CALL(cudaMallocManaged((void**)&frontiers_id, sizeof(int)*graph.vertices_count));
            int *bunched_id;
            SAFE_CALL(cudaMallocManaged((void**)&bunched_id, sizeof(int)*graph.vertices_count));
            int *low_id;
            SAFE_CALL(cudaMallocManaged((void**)&low_id, sizeof(int)*graph.vertices_count));

            // запускаем алгоритм
            //cudaDeviceSynchronize();
            t1 = omp_get_wtime();
            int last_source = 0;
            vector<cudaStream_t> Stream(3);
            for ( int i = 0; i < 3; i++ ) {
            ...
            for (int i = 0; i < iterations; i++) {
                #pragma omp critical
                {
                    ...
                    gpu_device = device;
                    gpu_last_source = last_source;
                }
                user_algorithm(gpu_graph, user_result + offset, frontiers_id, bunched_id, low_id, Stream, last_source);
            }
            t2 = omp_get_wtime();
        ...
        // делаем проверку случайно
        if(check)
        {
            verify_result(csr_graph, user_result + gpu_device*graph.vertices_count, gpu_last_source);
        }
