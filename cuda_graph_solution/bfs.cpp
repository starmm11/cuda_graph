...
void user_algorithm(GraphCSR _graph, int *_levels, int *_frontier_ids, int _source_vertex)
{
    ...
    gpu_bfs_wrapper(_graph.outgoing_ptrs, _graph.outgoing_ids, _graph.vertices_count, _graph.edges_count, _source_vertex, device_levels, _frontier_ids);
	...
	try
	{
        ...
        #pragma omp parallel num_threads(deviceCount)
        {
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                user_algorithm(gpu_graph, user_result + offset, frontiers_id, last_source);
            }
            t2 = omp_get_wtime();
